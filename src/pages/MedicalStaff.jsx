...
// import { UserPlus, UserRound, Mail, Loader2, ShieldCheck, Trash2 } from 'lucide-react';
...
//   // 1. Backend se Doctors fetch karne ka function
...
//       }
//     } catch (err) {
//       toast.error("Bahi, doctors load nahi ho sakay!");
//     } finally {
//       setLoading(false);
//     }
//   };
...
//   // 2. Naya Doctor add karne ka function
...
//         setShowForm(false); // Form band kar dein
...
//   return (
//     <div className="p-2 md:p-8 animate-in fade-in duration-500">
//       {/* Header Section */}
...
//           </h2>
//           <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mt-1">Manage Hospital Personnel</p>
//         </div>
        
//         <button 
//           onClick={() => setShowForm(!showForm)}
//           className="bg-blue-600 text-white px-8 py-4 rounded-2xl font-black text-[10px] flex items-center gap-3 hover:bg-slate-900 transition-all shadow-xl shadow-blue-500/20 active:scale-95"
//         >
...
//       {/* Add Doctor Form */}
//       {showForm && (
//         <div className="bg-white dark:bg-[#0f172a] p-8 rounded-[2.5rem] mb-10 border border-slate-100 dark:border-white/5 shadow-2xl animate-in slide-in-from-top duration-500">
...
//               <input 
//                 className="w-full p-5 rounded-2xl bg-slate-50 dark:bg-white/5 dark:text-white outline-none border border-slate-100 dark:border-white/10 font-bold focus:border-blue-600 transition-all"
//                 placeholder="Dr. Hussain Ali"
//                 value={formData.name}
...
//             </div>
//             <div className="space-y-2">
//               <label className="text-[10px] font-black text-slate-400 uppercase ml-2">Security Password</label>
//               <input 
//                 className="w-full p-5 rounded-2xl bg-slate-50 dark:bg-white/5 dark:text-white outline-none border border-slate-100 dark:border-white/10 font-bold focus:border-blue-600 transition-all"
//                 placeholder="••••••••"
//                 type="password"
...
//               />
//             </div>
//             <button type="submit" className="md:col-span-3 bg-blue-600 text-white p-6 rounded-2xl font-black uppercase tracking-widest hover:bg-slate-900 transition-all shadow-lg shadow-blue-600/20">
...
//           <p className="text-slate-400 font-black text-[10px] tracking-[0.3em] uppercase italic">Decrypting Staff Database</p>
//         </div>
//       ) : (
//         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
//           {doctors.length > 0 ? doctors.map((doc) => (
//             <div key={doc._id} className="bg-white dark:bg-[#0f172a]/40 p-8 rounded-[3rem] border border-slate-100 dark:border-white/5 flex flex-col items-center text-center group hover:border-blue-600/50 hover:shadow-2xl transition-all relative overflow-hidden">
//               <div className="h-24 w-24 bg-blue-600/10 rounded-[2rem] flex items-center justify-center text-blue-600 mb-6 group-hover:bg-blue-600 group-hover:text-white transition-all duration-500">
//                 <UserRound size={45} />
//               </div>
              
//               <h4 className="text-2xl font-black dark:text-white uppercase italic tracking-tighter leading-tight mb-2">
//                 {doc.name}
//               </h4>
//               <p className="text-xs font-bold text-slate-400 dark:text-slate-500 flex items-center gap-2 mb-6">
//                 <Mail size={14} className="text-blue-500" /> {doc.email}
//               </p>

//               <div className="flex items-center gap-2">
//                 <div className="px-5 py-2 bg-emerald-500/10 text-emerald-500 rounded-xl text-[9px] font-black uppercase tracking-widest border border-emerald-500/20">
//                   <ShieldCheck size={12} className="inline mr-1" /> Authorized
//                 </div>
//                 {/* Future mein delete button yahan add kar sakte hain */}
//               </div>
//             </div>
//           )) : (
//             <div className="col-span-full py-20 text-center border-2 border-dashed border-slate-200 dark:border-white/5 rounded-[3rem] opacity-50">
//                <p className="font-black text-xs uppercase tracking-widest text-slate-400 italic">No medical staff registered yet</p>
//             </div>
//           )}
//         </div>
//       )}
...
//   const [formData, setFormData] = useState({ name: '', email: '', password: '' });
//   const [editId, setEditId] = useState(null); // Track kar raha hai ke edit ho raha hai ya add
...
//   useEffect(() => {
//     fetchStaff();
//     resetForm(); // Tab change hote hi form band aur reset ho jaye
//   }, [activeTab]);
...
//         if (res.data.success) {
//           toast.success(`${activeTab.toUpperCase()} successfully registered!`);
//         }
//       }
//       resetForm();
//       fetchStaff();
...
//       name: member.name,
//       email: member.email,
//       password: '' // Password khali rakhenge taake agar change karna ho tabhi likhein
//     });
//     setShowForm(true);
...
//   // 1. Pehle confirm karein (Professional tareeke se window.confirm use karein ya sweetalert)
//   const confirmDelete = window.confirm("Are you sure you want to terminate this account? This action cannot be undone.");
  
//   if (!confirmDelete) return;

//   // 2. Loading state ke liye aik id-based toast dikhayein
...
//         render: "Account Terminated Successfully", 
//         type: "success", 
//         isLoading: false, 
//         autoClose: 3000 
//       });
//       fetchStaff(); // List refresh
//     }
//   } catch (err) {
//     toast.update(toastId, { 
//       render: err.response?.data?.message || "Action Denied", 
//       type: "error", 
//       isLoading: false, 
//       autoClose: 3000 
//     });
...
//   return (
//     <div className="p- md:p-4 animate-in fade-in duration-500">
//       {/* --- Header Section --- */}
//       <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
//         <div>
//           <h2 className="text-4xl font-black dark:text-white uppercase italic tracking-tighter">
...
//                 className={` cursor-pointer text-[10px] font-black uppercase tracking-widest pb-1 border-b-2 transition-all ${activeTab === tab ? 'border-blue-600 text-blue-600' : 'border-transparent text-slate-400'}`}
//               >
//                 {tab}s
//               </button>
//             ))}
...
//       {showForm && (
//         <div className="bg-white dark:bg-[#0f172a] p-8 rounded-[2.5rem] mb-10 border-2 border-blue-600/20 shadow-2xl animate-in zoom-in-95 duration-300">
//           <div className="mb-6">
//             <h3 className="text-xl font-black uppercase italic text-blue-600 cursor-pointer">
//               {editId ? `Edit ${activeTab} Details` : `Register New ${activeTab}`}
//             </h3>
//           </div>
...
//             </div>
//             <div className="space-y-2">
//               <label className="text-[10px] font-black text-slate-400 uppercase ml-2">Email Access</label>
//               <input 
//                 className="w-full p-5 rounded-2xl bg-slate-50 dark:bg-white/5 dark:text-white outline-none border border-slate-100 dark:border-white/10 font-bold focus:ring-2 ring-blue-600/20 transition-all"
//                 placeholder="hussain@clinic.com"
//                 type="email"
//                 value={formData.email}
//                 onChange={(e) => setFormData({...formData, email: e.target.value})}
//                 required
...
//                 {editId ? "New Password (Optional)" : "Portal Password"}
//               </label>
//               <input 
//                 className="w-full p-5 rounded-2xl bg-slate-50 dark:bg-white/5 dark:text-white outline-none border border-slate-100 dark:border-white/10 font-bold focus:ring-2 ring-blue-600/20 transition-all"
//                 placeholder={editId ? "Leave blank to keep same" : "Min. 6 characters"}
//                 type="password"
//                 value={formData.password}
//                 onChange={(e) => setFormData({...formData, password: e.target.value})}
//                 required={!editId}
//               />
//             </div>
//             <button type="submit" className="md:col-span-3 bg-blue-600 text-white p-6 rounded-2xl font-black uppercase tracking-widest hover:bg-blue-700 transition-all shadow-lg shadow-blue-600/30 cursor-pointer">
//               {editId ? "Update Staff Member" : `Complete ${activeTab} Registration`}
//             </button>
//           </form>
//         </div>
//       )}
...
//         <div className="py-40 text-center">
//           <Loader2 className="animate-spin mx-auto text-blue-600 mb-4" size={50} />
//           <p className="text-slate-400 font-black text-[10px] tracking-[0.3em] uppercase italic">Accessing {activeTab} Database</p>
//         </div>
//       ) : (
//         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
//           {staff.length > 0 ? staff.map((member) => (
//             <div key={member._id} className="bg-white dark:bg-[#0f172a]/40 p-8 rounded-[3rem] border border-slate-100 dark:border-white/5 flex flex-col items-center text-center group hover:border-blue-600 transition-all relative">
              
             
//               <div className="absolute top-6 right-6 flex gap-2 opacity-0 group-hover:opacity-100 transition-all">
//                 <button 
//                   onClick={() => handleEditClick(member)}
//                   className="p-3 bg-blue-500/10 text-blue-500 rounded-full hover:bg-blue-500 hover:text-white transition-all shadow-sm"
//                   title="Edit Staff"
//                 >
//                   <Edit3 size={16} />
//                 </button>
...
//                   className="p-3 bg-red-500/10 text-red-500 rounded-full hover:bg-red-500 hover:text-white transition-all shadow-sm"
//                   title="Remove Staff"
//                 >
//                   <Trash2 size={16} />
//                 </button>
...
//               <div className="h-24 w-24 bg-blue-600/10 rounded-[2rem] flex items-center justify-center text-blue-600 mb-6 group-hover:scale-110 transition-transform shadow-inner">
//                 {activeTab === 'doctor' ? <UserRound size={45} /> : <Users size={45} />}
//               </div>
...
//               </h4>
//               <p className="text-xs font-bold text-slate-400 dark:text-slate-500 flex items-center gap-2 mb-6">
//                 <Mail size={14} className="text-blue-500" /> {member.email}
//               </p>

//               <div className="px-5 py-2 bg-emerald-500/10 text-emerald-500 rounded-xl text-[9px] font-black uppercase tracking-widest border border-emerald-500/20">
//                 <ShieldCheck size={12} className="inline mr-1" /> Verified {activeTab}
//               </div>
//             </div>
//           )) : (
//             <div className="col-span-full py-20 text-center border-2 border-dashed border-slate-200 dark:border-white/5 rounded-[3rem]">
//                <p className="font-black text-xs uppercase tracking-widest text-slate-400 italic">No {activeTab}s Found in Records</p>
//             </div>
//           )}
//         </div>
//       )} */}
